...
Plugin/DLL cho **AutoCAD** (ObjectARX 2022) thực hiện **UV mapping** cho bề mặt: sinh lưới UV, mapping theo alignment/polyline, và unwrap UV bằng [xatlas](https://github.com/jpcy/xatlas). Xuất dữ liệu hình học ra JSON/binary.
...
├── ObjectARX/2022/            # ⚠️ TỰ TẢI (không có trong repo, đã .gitignore)
│   ├── inc/  inc-x64/
│   └── lib-x64/
...
|---------------|----------|
| **AutoCadLib** | Tiện ích chung: `CAcadUtil`, `CGeometryUtil`, `CommonDefined`, `CAcadOpenForWrite` |
...
| **UVMap** | Tính năng: `AutoCadUVMap`, `CJsonAdapter` |
...
- **ObjectARX** KHÔNG đi kèm repo (SDK độc quyền Autodesk, cấm phân phối lại) → bạn tự tải. Nhờ vậy repo **có thể để Public**.
